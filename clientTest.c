    ...
    while (message_flag) {
        ...
        if (receive > 0) {
            ...
            if (received_message == NULL) {
                ...
                exit(EXIT_FAILURE); // Handle allocation failure appropriately
            }
...
// //---------------------- in prog -------------------------------
// char *receive_messages(void *arg) {
//     Client *client = (Client *)arg;
//     int message_flag = 1;
//     char message[MAX_MESSAGE_LENGTH];
...
//             client->received_message = (char *)malloc(receive + 1); // +1 for null terminator
//             if (client->received_message == NULL) {
//                 perror("Error allocating memory for received message");
//                 exit(EXIT_FAILURE); // Handle allocation failure
//             }
//             strncpy(client->received_message, message, receive);
//             client->received_message[receive] = '\0'; // Null-terminate the string
